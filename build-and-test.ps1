...
<#
.SYNOPSIS
    Clean-rebuilds every configuration of BAnalyzer.sln and runs the
...
# Paths
# ---------------------------------------------------------------------------
$SolutionFile = Join-Path $PSScriptRoot "BAnalyzer.sln"
...
foreach ($Cfg in $Configurations) {
    ...
    Write-Progress -Activity "BAnalyzer build+test" `
...
Write-Progress -Activity "BAnalyzer build+test" -Completed
...
Write-Host ""
Write-Host ("=" * 70)
Write-Host "## Build & Test Report -- BAnalyzer -- $Timestamp"
Write-Host ("=" * 70)
Write-Host ""
